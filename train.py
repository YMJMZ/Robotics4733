...
from action_regression_model import recover_action

from image import read_rgb, read_mask
...
class RGBDataset(Dataset):
    def __init__(self, labels_dir: str):
        ...
        self.masks_dir = os.path.join(labels_dir[:-6], 'scoremaps')
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        score_path = os.path.join(self.masks_dir, '{}_scoremap.png'.format(key))
        rgb = read_rgb_cached(img_path)
        scoremap = read_mask(score_path)
        # modification
        ...
        data = {
            ...
            'angle': angle,
            'scoremap': scoremap/np.max(scoremap)
        }
        data_torch = dict()
...
def get_center_angle(
    ...
    # ===============================================================================
    return center_coord, angle.astype(np.float32)
...
class AugmentedDataset(Dataset):
    ...
    def __getitem__(self, idx: int
        ...
        scoremap = data_torch['scoremap'].numpy()
        left_point, right_point = get_finger_points(center_point, angle)
        kps = KeypointsOnImage([
            Keypoint(x=left_point[0], y=left_point[1]),
            Keypoint(x=right_point[0], y=right_point[1])
        ], shape=rgb.shape)

        image_aug, kps_aug = self.aug_pipeline(
            image=np.concatenate([np.expand_dims(scoremap, axis=-1), rgb], axis=-1), 
            keypoints=kps
            )
        aug_center, aug_angle = get_center_angle(
            np.array([kps_aug[0].x, kps_aug[0].y]), np.array([kps_aug[1].x, kps_aug[1].y]))
        data_torch['rgb'] = torch.from_numpy(image_aug[:,:,1:])
        data_torch['center_point'] = torch.from_numpy(aug_center)
        data_torch['angle'] = torch.tensor(aug_angle)
        data_torch['scoremap'] = torch.from_numpy(image_aug[:,:,0])
        # ===============================================================================
        return data_torch
...
if __name__ == "__main__":
    ...
    dataset_dir = './data/labels'
    raw_dataset = RGBDataset(dataset_dir)
    train_raw_dataset = AugmentedDataset(raw_dataset)
    train_dataset = affordance_model.AffordanceDataset(train_raw_dataset)

    # num = 5
    # fig, ax = plt.subplots(num, 2, figsize=(20,20))
    # for i in range (num):
    #     n = np.random.randint(len(raw_dataset))
    #     t = train_dataset[n]
    #     rgb_obs = t['input'].permute(1,2,0).numpy()
    #     scoremap = t['target'].numpy()
    #     ax[i, 0].imshow(rgb_obs)
    #     ax[i, 1].imshow(scoremap[0,:,:])
    t = train_dataset[45]
    scoremap = t['target'].numpy()
    plt.imshow(scoremap[0,:,:])
    plt.show()
